...
query = " Hotel near the Louvre with great food nearby."  # A
query_embedding = model.encode([query])  # B

k = 5  # Number of similar reviews to retrieve #C
indices, distances = cosine_search(query_embedding, review_embeddings, k)


print(f"Query: {query}")  # A
print("Top hotel with similar reviews matching the query:")  # B
for i, (idx, distance) in enumerate(zip(indices, distances), 1):  # C
    print(f"{i}. {df_paris.iloc[idx]['hotel_name']}")  # D
    print(f"Review: {df_paris.iloc[idx]['review_text']}")  # E
    print(f"Distance: {distance:.4f}")  # F
    print()

    # A We print the query text.
    # B We print a header indicating that we are displaying the top hotels with similar reviews.
    # C We iterate over the top-k similar reviews using `enumerate()` and `zip()` to pair the indices and distances (pair in a tupple).
    # D For each similar review:
    # E We print the hotel name using `df_paris.iloc[idx]['hotel_name']`, where `idx` is the index of the similar review.
    # F We print the review text using `df_paris.iloc[idx]['review_text']`.”
